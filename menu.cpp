...
void Menu::draw() const {
  int x = position[0];
  int y = position[1];
  io.printMessageAt("Ghost Blaster !", 350, 50);
...
void Menu::drawHelp() const {
  io.printMessageAt("Press F1 to toggle HUD", 10, 20);
  io.printMessageAt("Press a or d to move Ghost Rider's direction", 10, 30);
  io.printMessageAt("Press space to shoot", 10, 40);
  io.printMessageAt("Press m to toggle music", 10, 50);
  io.printMessageAt("SHOOT 8 enemies to win the game before you die !", 10, 60);
}
